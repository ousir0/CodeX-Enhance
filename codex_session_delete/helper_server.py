...
class ExportService(Protocol):
    def export(self, session: SessionRef, filename: str | None = None, export_dir: str | None = None) -> ExportResult: ...
...
class HelperServer(ThreadingHTTPServer):
    def __init__(
        ...
        *,
        ...
        backend_status_handler: Callable[[], dict[str, object]] | None = None,
        backend_repair_handler: Callable[[], dict[str, object]] | None = None,
    ):
        self.service = service
        self.export_service = export_service
        self.allow_http_mutation = allow_http_mutation
        self.http_mutation_token = http_mutation_token
        self.backend_status_handler = backend_status_handler
        self.backend_repair_handler = backend_repair_handler
    ...
    def do_POST(self) -> None:
        try:
            ...
            if self.path == "/backend/status":
                handler = self.server.backend_status_handler
                self._send_json(handler() if handler else {"status": "ok", "message": "后端已连接"})
                return
            if self.path == "/backend/repair":
                handler = self.server.backend_repair_handler
                self._send_json(handler() if handler else {"status": "failed", "message": "后端修复不可用"}, status=200 if handler else 400)
                return
            ...
            if self.path == "/export-markdown":
                if self.server.export_service is None:
                    ...
                    return
                session = SessionRef(session_id=str(payload.get("session_id", "")), title=str(payload.get("title", "")))
                self._send_json(self.server.export_service.export(session, filename=str(payload.get("filename", "")).strip() or None).to_dict())
                return
