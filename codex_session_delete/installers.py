...
def install_codex_plus_plus(options: InstallOptions) -> None:
    ...
    if sys.platform == "win32":
        ...
        return
    raise RuntimeError(f"Unsupported platform for {PRODUCT_NAME} install: {sys.platform}")
...
def uninstall_codex_plus_plus(options: InstallOptions) -> None:
    ...
    if sys.platform == "win32":
        ...
        if options.remove_data:
            remove_owned_data()
        return
    raise RuntimeError(f"Unsupported platform for {PRODUCT_NAME} uninstall: {sys.platform}")
