...
from codex_session_delete import ASCII_PRODUCT_NAME, __version__

DEFAULT_REPOSITORY = os.environ.get("CODEX_PLUS_UPDATE_REPOSITORY", "").strip()
DEFAULT_RELEASE_API_URL = f"https://api.github.com/repos/{DEFAULT_REPOSITORY}/releases/latest" if DEFAULT_REPOSITORY else ""
USER_AGENT = f"{ASCII_PRODUCT_NAME}/{__version__}"
...
def fetch_latest_release(api_url: str | None = None, timeout: int = 10) -> Release:
    release_api_url = api_url or DEFAULT_RELEASE_API_URL
    if not release_api_url:
        raise UpdateError("未配置 GitHub Release 仓库；请设置 CODEX_PLUS_UPDATE_REPOSITORY，例如 owner/repo。")
    response = requests.get(release_api_url, timeout=timeout, headers={"User-Agent": USER_AGENT, "Accept": "application/vnd.github+json"})
