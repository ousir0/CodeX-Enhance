...
from codex_session_delete import PRODUCT_NAME
...
def create_backup(home: Path, target_provider: str, changes: list[SessionChange]) -> Path:
    ...
    (backup_dir / "metadata.json").write_text(
        json.dumps({"managedBy": f"{PRODUCT_NAME} provider sync", "targetProvider": target_provider}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    return backup_dir
...
def prune_backups(home: Path, keep_count: int = BACKUP_KEEP_COUNT) -> None:
    ...
    for path in backup_root.iterdir():
        ...
        try:
            ...
            continue
        if metadata.get("managedBy") in {f"{PRODUCT_NAME} provider sync", "Codex++ provider sync"}:
            managed.append(path)
