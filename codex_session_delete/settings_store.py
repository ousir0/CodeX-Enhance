...
class BackendSettings:
    ...
    skip_login_enabled: bool = True
    export_directory: str = ""

    @classmethod
    def from_dict(cls, data: dict[str, object]) -> "BackendSettings":
        return cls(
            provider_sync_enabled=bool(data.get("providerSyncEnabled", False)),
            skip_login_enabled=bool(data.get("skipLoginEnabled", True)),
            export_directory=str(data.get("exportDirectory", "")).strip(),
        )

    def to_dict(self) -> dict[str, object]:
        return {
            "providerSyncEnabled": self.provider_sync_enabled,
            "skipLoginEnabled": self.skip_login_enabled,
...
class SettingsStore:
    ...
    def update(self, values: dict[str, object]) -> BackendSettings:
        ...
        if "skipLoginEnabled" in values:
            current["skipLoginEnabled"] = bool(values["skipLoginEnabled"])
        if "exportDirectory" in values:
            current["exportDirectory"] = str(values["exportDirectory"]).strip()
