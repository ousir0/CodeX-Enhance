...
def _launcher_command(options: "InstallOptions") -> str:
    return options.launcher_command or f"{_default_python_executable()} -m codex_session_delete unlock"
...
$ShortcutPath = Join-Path $InstallRoot 'CodeX 增强.lnk'
...
$Shortcut.Description = 'Launch Codex with CodeX Enhance injection'
...
$LegacyUninstallKey = 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\CodexPlusPlus'
if (Test-Path $LegacyUninstallKey) {{ Remove-Item $LegacyUninstallKey -Force }}
$UninstallKey = 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\CodeXEnhance'
...
Set-ItemProperty -Path $UninstallKey -Name DisplayName -Value '{PRODUCT_NAME}（{PRODUCT_SUBTITLE}）'
Set-ItemProperty -Path $UninstallKey -Name DisplayVersion -Value '{__version__}'
Set-ItemProperty -Path $UninstallKey -Name Publisher -Value '{ASCII_PRODUCT_NAME}'
